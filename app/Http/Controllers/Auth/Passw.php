...
class Passw implements Rule, DataAwareRule, ValidatorAwareRule
{
    use Conditionable;
    ...
    public function passes($attribute, $value)
    {
        ...
        $validator = Validator::make(
            ...
            if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
                $validator->errors()->add($attribute, ' رمز عبور باید حداقل یک حرف بزرگ و یک حرف کوچک داشته باشد');
            }

            if ($this->letters && ! preg_match('/\pL/u', $value)) {
                $validator->errors()->add($attribute, ' رمز عبور باید حداقل یک حرف داشته باشد');
            }

            if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
                $validator->errors()->add($attribute, ' رمز عبور باید حداقل یک علامت داشته باشد');
            }

            if ($this->numbers && ! preg_match('/\pN/u', $value)) {
                $validator->errors()->add($attribute, ' رمز عبور باید حداقل یک عدد داشته باشد');
            }
        });
